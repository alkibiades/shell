...
void eval(char *cmdline) 
{
	...
	if(!builtin_cmd(argv)){
		pid_t pid=fork();
		if(pid==0){
			setpgid(0,0);	/*group ID <-- child's PID.*/
...
void do_bgfg(char **argv) 
{
	...
	//	    printf("%d",jid);
		    kjob=getjobjid(jobs,jid);
		    kill(kjob->pid,SIGCONT);
			printf("[%d] (%d) %s",kjob->jid,kjob->pid,kjob->cmdline);
		    kjob->state=BG;
	    }
    ...
    if(!strcmp(argv[0],"fg")){
		...
		if(kjob->state==ST){
			...
			kill(kjob->pid,SIGCONT);
			int stat;
			waitpid(kjob->pid,&stat,WUNTRACED);
			deletejob(jobs,kjob->pid);
		}
		else if(kjob->state==BG){
			kjob->state=FG;
		}
	    }
    }
